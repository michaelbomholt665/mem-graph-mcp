...
- [ ] Add graph-query tool models for node snapshots, edge snapshots, and node-detail payloads.
- [ ] Implement `get_graph_snapshot`, `get_node_details`, and `search_graph` against the existing graph client with bounded depth and filtering.
- [ ] Define stable resource and route names so the dashboard can fetch graph data without coupling to internal tool names.
- [ ] Build a static dashboard shell with a canvas, filter controls, search input, and a details panel.
- [ ] Implement dashboard JavaScript for initial load, periodic refresh, node selection, search focus, and type filtering.
- [ ] Add CSS that keeps the dashboard readable on laptop screens without assuming a full design-system rewrite.
- [ ] Add node-style metadata so graph types are colored and sized consistently across refreshes.
- [ ] Mount dashboard routes and static assets in the server without breaking existing MCP endpoints.
- [ ] Add tests for graph snapshot shaping, query filtering, and route responses.
- [ ] Document how to launch the dashboard, what node types are visible, and what is intentionally deferred.
...
- [ ] A browser route renders the dashboard and loads graph data from server APIs.
- [ ] Search, node selection, and type filters work on real graph data.
- [ ] The dashboard remains responsive on moderately sized graphs.
- [ ] The implementation leaves room for Jira and file-explorer tabs instead of boxing them out.
