#!/usr/bin/env python3
"""Read-only Jira integration tools."""
...
from ...services.jira_embedder import (
    JiraConfigurationError,
    get_jira_embedder,
)

mcp = FastMCP("integrations", instructions="Read-only Jira issue fetch and ticket-to-code linking tools.")

_TAG = {"namespace:integrations"}


@mcp.tool(tags=_TAG)
async def jira_fetch_issues(
    jql: Annotated[
        str | None,
    ...
    project_id: Annotated[
        str | None,
        Field(description="Optional project node ID used to link imported Jira issues into the graph."),
    ] = None,
) -> dict:
    """Fetch Jira issues over the read-only Jira search API and persist them in the graph."""
    ...
    try:
        ...
        return {"error": f"Failed to fetch Jira issues: {exc}"}
...
@mcp.tool(tags=_TAG)
async def jira_find_code_for_ticket(
    issue_key: Annotated[str, Field(description="Jira issue key, for example MEM-42.")],
    root_path: Annotated[
        str | None,
    ...
    """Fetch a Jira issue and find the most relevant code files for it."""
    embedder = get_jira_embedder()
    try:
        issue = await embedder.fetch_issue(issue_key)
        if issue is None:
            return {"error": f"Jira issue {issue_key!r} was not found."}
        ...
        return {"error": f"Failed to link Jira issue to code: {exc}"}
...
@mcp.tool(tags=_TAG)
async def jira_find_tickets_for_file(
    ...
    limit: Annotated[int, Field(description="Maximum number of Jira matches to return.", ge=1, le=20)] = 5,
    threshold: Annotated[
        float | None,
        Field(description="Optional semantic match threshold override between 0 and 1.", ge=0.0, le=1.0),
    ] = None,
    include_resolved: Annotated[
        bool,
        Field(description="Include closed or resolved Jira issues in the result set."),
    ] = True,
) -> dict:
    """Find persisted Jira issues related to a specific file."""
    embedder = get_jira_embedder()
    try:
        ...
        return {"error": f"Failed to find Jira issues for file: {exc}"}
